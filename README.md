...
- **This repo** = Next.js app (sidebar, markdown rendering, auth, theme). No project content is committed here; `content/` holds only a README.
- **Content repos** = `site.config.json` + `content/` with section dirs (`adr/`, `designs/`, `guides/`, etc.). Each project (Numanity, TPG, etc.) has its own content repo.
...
```bash
rm -rf content
ln -s /path/to/numanity-docs content
...
- Next.js 15 (App Router), Tailwind, NextAuth (Google), react-markdown, Scalar API Reference (optional). See `site.config.ts` and content-repo `site.config.json` for sections, tabs, theme, and feature flags.
